			 ...
			 MLIFETiming *timedata, void *privateData )
{
    ...
    if(patch->left != -1){
    MPI_Win_lock(MPI_LOCK_SHARED, patch->left, 0, win);
    disp = (left_LCols + 2) + (left_LCols + 1);
    MPI_Put(&matrix[1][1], 1, mytype, patch->left, disp, 1, 
	    left_type, win);
    MPI_Win_unlock(patch->left, win);
    }

    if(patch->right != -1){
    MPI_Win_lock(MPI_LOCK_SHARED, patch->right, 0, win);
    disp = right_LCols + 2;
    MPI_Put(&matrix[1][LCols], 1, mytype, patch->right, disp, 1, 
            right_type, win); 	
    MPI_Win_unlock(patch->right, win);
    }
    ...
    /* now put the top, bottom edges (including the diagonal
       points) */ 
    if(patch->up != -1) {
    MPI_Win_lock(MPI_LOCK_SHARED, patch->up, 0, win);
    MPI_Put(&matrix[1][0], LCols + 2, MPI_INT, patch->up,
            (above_LRows+1)*(LCols+2), LCols+2, MPI_INT, win);
    MPI_Win_unlock(patch->up, win);
    }
    if(patch->down != -1){
    MPI_Win_lock(MPI_LOCK_SHARED, patch->down, 0, win);
    MPI_Put(&matrix[LRows][0], LCols + 2, MPI_INT, patch->down, 0,
            LCols+2, MPI_INT, win);
    MPI_Win_unlock(patch->down, win);
    }
